...
font-size: 16px;"> Last access : 30 May 2014 &nbsp; <a href="/foot/admin.php/home/user/logout" class="btn btn-danger square-btn-adjust">Logout</a> </div>
		...
		<div class="col-md-12">
					<form  action="<?php echo U('Access/accessnode');?>" method="POST">
          <input type="hidden" name="role_id" value="<?php echo ($rid); ?>">
          <?php if(is_array($access)): foreach($access as $key=>$arr): ?><input type="checkbox" name="node_id[]"  value="<?php echo ($arr["id"]); ?>"><?php echo ($arr["name"]); ?>&nbsp<?php endforeach; endif; ?>
								...
								<div class="form-group">
									<button class="btn btn-info" onclick="$('#node').submit()">submit</button>
